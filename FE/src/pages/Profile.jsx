...
// }
import { useState, useEffect } from 'react';
import { initialMockData } from '../mockData.js'; // Đồng bộ trực tiếp với file mock data chung
...
export default function Profile() {
  ...
  // TỰ ĐỘNG PHÁT HIỆN ROLE THEO URL NẾU KHÔNG CÓ DATA BACKEND
  const isInstructorRoute = window.location.pathname.includes('/instructor');
  const fallbackRole = isInstructorRoute ? 'INSTRUCTOR' : 'STUDENT';
  ...
  const fetchUserProfile = () => {
    setLoading(true);
    try {
      // Đọc thông tin user hiện tại từ mock data hub
      const data = initialMockData.userProfile || {
        id: "usr_mock_999",
        firstName: "Alexander",
        lastName: "Instructor",
        email: "alex.instructor@institution.edu",
        role: fallbackRole
      };
  ...
  const handleUpdateProfile = (e) => {
    ...
    // Giả lập độ trễ cập nhật dữ liệu qua mạng nội bộ
    setTimeout(() => {
      try {
        const updatedUser = {
          ...user,
        ...
        // Ghi đè ngược lại vào Mock Data Hub để lưu trạng thái runtime toàn cục
        initialMockData.userProfile = updatedUser;
        setUser(updatedUser);
        
        showAlert("success", "Profile metadata clusters updated successfully on backend core.");
      } catch (error) {
        showAlert("error", "Validation error detected during profile synchronization.");
      } finally {
        setSubmitting(false);
      }
    }, 400);
    ...
    <div className="min-h-screen bg-[#f8fafc] text-[#0f172a] p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 border-b border-gray-200 pb-6">
          <h1 className="text-3xl font-black text-[#1e3a8a] tracking-tight">Account Parameters</h1>
          <p className="text-xs text-gray-400 mt-1">
            Manage your personal cryptographic identification, profile identities, and institutional platform authority. (MOCK ACTIVE)
          </p>
        </div>
