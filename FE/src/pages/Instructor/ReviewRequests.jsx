...
// }
import { useState, useEffect } from 'react';
import { initialMockData } from '../../mockData.js'; // Đồng bộ dữ liệu trực tiếp từ mock data hub chung
...
export default function ReviewRequests() {
  ...
  // Lấy toàn bộ danh sách feedback requests giả lập từ mock data hub
  ...
  // Cập nhật trạng thái vòng lặp chấm điểm giả lập trực tiếp trên Client State
  ...
  // Ghi nhận xét giả lập và phản hồi hệ thống thông báo nhanh
  const handleSubmitComment = (e) => {
    ...
    // Giả lập độ trễ phản hồi của hệ thống
    setTimeout(() => {
      try {
        alert("Instructor diagnostic feedback dispatched successfully (Mock Simulation Saved)!");
        setFeedbackContent("");
        setSelectedRequest(null);
      } catch (error) {
        setErrorMessage("Failed to attach textual comment payload onto verification node structure.");
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-8 text-[#0f172a]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 border-b border-gray-200 pb-6">
          <h1 className="text-3xl font-black text-[#1e3a8a] tracking-tight">Student Review Queue</h1>
          <p className="text-xs text-gray-400 mt-1">Audit AI evidence assertions, evaluate project models, and emit authoritative grading verdicts. (MOCK ACTIVE)</p>
        </div>
