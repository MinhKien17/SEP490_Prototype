...
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { initialMockData } from '../../mockData.js'; // Đồng bộ trực tiếp từ mock data hub chung
...
export default function CollectionList() {
  ...
  // Tải danh sách project và các collection giả lập ban đầu
  const fetchInitialData = () => {
    ...
    try {
      ...
      if (projectList.length > 0) {
        const firstId = projectList[0].id;
        setSelectedProjectId(firstId);
        // Lấy danh sách mockup collections chung để test giao diện
  ...
  // Thay đổi bộ lọc hiển thị theo từng dự án (Giả lập xáo trộn dữ liệu để test giao diện)
  const handleProjectFilterChange = (pId) => {
    setSelectedProjectId(pId);
    if (!pId) return;
    setLoading(true);
    
    // Giả lập lật dữ liệu ngẫu nhiên hoặc giữ nguyên danh sách mẫu để hiển thị
    setTimeout(() => {
      if (pId === 'proj_102') {
        // Trả về mảng rỗng cho project 102 để test giao diện empty state
        setCollections([]);
      } else {
        setCollections(initialMockData.collections || []);
      }
  ...
  // Giả lập xóa bộ sưu tập mẫu trực tiếp trên State mạng nội bộ FE
    ...
    <div className="min-h-screen bg-[#f8fafc] p-8 text-[#0f172a]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-gray-200 pb-6">
          <div>
            <h1 className="text-3xl font-black text-[#1e3a8a] tracking-tight">Evidence Libraries</h1>
            <p className="text-xs text-gray-400 mt-1">Configure baseline compliance parameters, document templates, and scope evaluation rules. (MOCK ACTIVE)</p>
          </div>
          <button
            onClick={() => navigate('/instructor/collections/create')}
            className="px-5 py-2.5 bg-[#1e3a8a] text-white font-black text-xs rounded-xl hover:bg-blue-800 transition shadow-sm"
