...
export default function Workspace() {
  ...
  // States mới cho tính năng bổ sung
...
Some teams assume daily meetings alone are enough to control project uncertainty, but this claim still needs stronger evidence from the uploaded sources.`);
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-slate-50 overflow-hidden font-sans antialiased text-slate-800">
      ...
      <header className="h-14 border-b border-slate-200 bg-white/80 backdrop-blur-md flex items-center justify-between px-4 shrink-0 shadow-sm z-20">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="text-xs font-semibold text-rose-600 hover:text-rose-700 border border-rose-200 px-3 py-1.5 rounded-lg hover:bg-rose-50 transition-all shadow-sm ml-2"
          >
            Log Out
          </button>
        </div>
                  ...
                  {project ? (
                    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-bold text-base text-slate-800">{project.title}</h3>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase ${project.status === 'IN_REVIEW' ? 'bg-amber-50 text-amber-700 border-amber-200' : project.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{project.status}</span>
                      </div>
                      <p className="text-xs text-slate-600 mb-4 leading-relaxed">{project.description || "Chưa có mô tả cho dự án này."}</p>
                      
                      <div className="text-[11px] text-slate-400 space-y-1.5 border-t border-slate-100 pt-3">
                        ...
                        <div className="flex justify-between">
                          <span>Ngày tạo:</span>
                          <span className="font-semibold text-slate-700">{project.createdAt ? new Date(project.createdAt).toLocaleDateString('vi-VN') : 'N/A'}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 mt-5">
                        <button 
                          ...
                          className="flex-1 py-1.5 text-xs font-semibold text-rose-600 hover:text-white border border-rose-200 hover:bg-rose-600 rounded-lg transition-colors"
                        >
                          Xóa dự án
                        </button>
                        <button 
                          onClick={() => {
                            const newTitle = prompt("Nhập tiêu đề mới:", project.title);
                            if (!newTitle) return;
                            const newDesc = prompt("Nhập mô tả mới:", project.description || "");
                            api.put(`/api/projects/${project.id}`, { title: newTitle, description: newDesc })
                              .then(() => {
                          ...
                          className="flex-1 py-1.5 text-xs font-semibold text-slate-600 hover:text-slate-800 border border-slate-200 hover:bg-slate-50 rounded-lg transition-colors"
                        >
                          Sửa thông tin
                        </button>
                      </div>
                    ...
                    <div className="text-sm text-slate-500 italic text-center py-8">Vui lòng chọn hoặc tạo dự án để bắt đầu.</div>
                  )}

                  <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                    <h4 className="font-bold text-xs text-slate-400 uppercase tracking-wider mb-3">Tài nguyên được chia sẻ</h4>
                    <div className="space-y-2.5">
                      <div className="p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                        <p className="text-xs font-bold text-slate-700 flex items-center gap-1.5"><svg className="w-3.5 h-3.5 text-red-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd"/></svg>instructor-agile-risk-framework.pdf</p>
                        <p className="text-[10px] text-slate-400 mt-1">Tài liệu khung quản lý rủi ro agile được chỉ định bởi giảng viên.</p>
                      </div>
                      <div className="p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                        <p className="text-xs font-bold text-slate-700 flex items-center gap-1.5"><svg className="w-3.5 h-3.5 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd"/></svg>feedback-loop-benchmark.docx</p>
                        <p className="text-[10px] text-slate-400 mt-1">Các tiêu chí đánh giá vòng lặp phản hồi sprint hiệu quả.</p>
                      </div>
                    </div>
                  </div>
               </div>
               ...
               <div className="flex flex-col gap-4 animate-in fade-in duration-200">
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm">
                    <div className="flex gap-2">
                      <input 
                        type="text" 
                        placeholder="Thêm luận điểm cần kiểm chứng..." 
                        value={newClaimContent}
                        onChange={(e) => setNewClaimContent(e.target.value)}
                        onKeyDown={(e) => { if(e.key === 'Enter') handleCreateClaim(); }}
                        className="flex-1 text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-transparent"
                      />
                      <button 
                        onClick={handleCreateClaim}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold shrink-0 transition-colors"
                      >
                        Thêm
                      </button>
                    </div>
                  ...
                  {/* Edit Claim Inline Form */}
                  {editingClaim && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 shadow-sm">
                      <p className="text-[10px] font-bold text-amber-700 uppercase tracking-wider mb-2">Chỉnh sửa luận điểm</p>
                      <textarea
                        value={editClaimContent}
                        onChange={(e) => setEditClaimContent(e.target.value)}
                        className="w-full text-xs border border-amber-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-transparent mb-2"
                        rows={2}
                      />
                      <div className="flex justify-end gap-1.5">
                        <button 
                          onClick={() => setEditingClaim(null)} 
                          className="px-2.5 py-1 text-[11px] text-slate-500 hover:bg-slate-100 rounded font-medium transition-colors"
                        >
                          Hủy
                        </button>
                        <button 
                          onClick={handleUpdateClaim} 
                          className="px-2.5 py-1 text-[11px] bg-amber-600 hover:bg-amber-700 text-white rounded font-bold transition-colors"
                        >
                          Lưu
                        </button>
                      </div>
      ...
      {showAiReviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl p-6 transform transition-all overflow-hidden flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center mb-4 shrink-0">
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <svg className="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 01-2 2h0a2 2 0 01-2-2v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                AI Review - Đánh giá cấu trúc & Định dạng bài báo
              </h2>
              <button onClick={() => { setShowAiReviewModal(false); setAiReviewResult(null); }} className="text-slate-400 hover:text-slate-600 transition-colors">
                 <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
            </div>
            
            <div className="flex-1 overflow-y-auto pr-1 space-y-4 my-2 custom-scrollbar text-sm">
              {loadingAiReview ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  <div className="w-10 h-10 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
                  <p className="text-slate-500 font-medium animate-pulse">Trợ lý AI đang đọc nội dung bài viết và phân tích định dạng...</p>
                </div>
              ) : aiReviewResult ? (
                <div className="space-y-4">
                  <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
                    <h3 className="font-bold text-indigo-900 mb-2 flex items-center gap-1.5">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      Đánh giá phong cách biên soạn (Style Feedback)
                    </h3>
                    <p className="text-slate-700 leading-relaxed whitespace-pre-line bg-white/70 p-3 rounded-lg border border-slate-100">{aiReviewResult.styleFeedback}</p>
                  </div>
                  <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
                    <h3 className="font-bold text-emerald-900 mb-2 flex items-center gap-1.5">
                      ...
                      Đánh giá cấu trúc học thuật (Structure Feedback)
                    </h3>
                    <p className="text-slate-700 leading-relaxed whitespace-pre-line bg-white/70 p-3 rounded-lg border border-slate-100">{aiReviewResult.structureFeedback}</p>
                  </div>
                </div>
              ) : (
                <p className="text-slate-400 italic text-center py-8">Chưa có kết quả phân tích.</p>
              )}
            </div>

            <div className="flex justify-end gap-3 pt-3 border-t border-slate-100 shrink-0">
              <button 
                onClick={() => { setShowAiReviewModal(false); setAiReviewResult(null); }} 
      ...
      {showSubmitReviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform transition-all">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-slate-800">Gửi dự án phê duyệt</h2>
              <button onClick={() => setShowSubmitReviewModal(false)} className="text-slate-400 hover:text-slate-600 transition-colors">
                 <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
            </div>
            <p className="text-sm text-slate-600 mb-4">Chọn giảng viên chịu trách nhiệm đánh giá và đối chiếu dự án của bạn:</p>
            
            <div className="space-y-4 mb-6">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Chọn Giảng viên</label>
                <select
                  value={selectedInstructorId}
                  onChange={(e) => setSelectedInstructorId(e.target.value)}
                  className="w-full text-xs border border-slate-300 rounded-xl px-3 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 shadow-sm"
                >
                  <option value="">-- Chọn giảng viên từ danh sách --</option>
                  <option value="2">Giảng viên Nguyễn Văn A (ID: 2)</option>
                  <option value="3">Giảng viên Trần Thị B (ID: 3)</option>
                  <option value="4">Giảng viên Lê Hoàng C (ID: 4)</option>
                </select>
              </div>

              <div className="relative flex py-1 items-center">
                <div className="flex-grow border-t border-slate-200"></div>
                <span className="flex-shrink mx-4 text-slate-400 text-[10px] font-bold uppercase">Hoặc tự nhập ID</span>
                <div className="flex-grow border-t border-slate-200"></div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Mã số giảng viên (Instructor ID)</label>
                <input
                  type="number"
                  placeholder="Nhập ID giảng viên, ví dụ: 2"
                  value={selectedInstructorId}
                  onChange={(e) => setSelectedInstructorId(e.target.value)}
                  className="w-full text-xs border border-slate-300 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 shadow-sm"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
              ...
              <button 
                ...
                className="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg shadow-sm shadow-indigo-200 transition-colors"
              >
                Gửi phê duyệt
              </button>
            </div>
